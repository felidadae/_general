...
perl -i -pe 'if (/^stage/) {s/font-size:.*?;/font-size: 10pt;/g;}' gnome-shell.css 
#search entry rounded looks so badly
perl -i -pe 'if (/^search-entry/) {s/border-radius:/s.*?;/border-radius: 0px;/g;}' gnome-shell.css 
